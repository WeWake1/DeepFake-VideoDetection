...
# Ensure we have a 'confidence' column. Some older scripts use 'score' (probability of FAKE)
# while newer versions wrote a 'confidence' directly (confidence in the predicted label).
if 'confidence' not in predictions_df.columns:
    if 'score' in predictions_df.columns and 'prediction' in predictions_df.columns:
        print("  Note: 'confidence' column not found - deriving from 'score' and 'prediction' columns")
        # score is P(FAKE). We want confidence in the predicted label:
        # confidence = score for predicted FAKE, 1-score for predicted REAL.
        predictions_df['confidence'] = np.where(predictions_df['prediction'] == 1,
                                                predictions_df['score'],
                                                1.0 - predictions_df['score'])
    else:
        raise KeyError("predictions.csv must contain either 'confidence' or ('score' and 'prediction') columns")

print(f"✓ Loaded {len(predictions_df)} predictions (using 'confidence' column)")
print()
...
for idx, (i, row) in enumerate(random_real.iterrows(), 1):
    ...
    if video_dir.exists():
        ...
        result = inference.predict_video(str(video_dir))
