"""
FF++ Face Extraction Script (OPTIMIZED)
========================================
Purpose: Extract faces from FF++ frame directories using MTCNN GPU with BATCH PROCESSING

Input:
- Real frames: F:/FF++_preprocessed/frames/FF Real Frames
- Fake frames: F:/FF++_preprocessed/frames/FF Fake Frames (6 manipulation methods)
...
- Skip videos that already have faces extracted (RESUME capability)
- Handles all 6 manipulation methods automatically
...
BATCH_SIZE = 32  # Process 16 frames at once for speed
...
def process_dataset(frames_dir, output_dir, label, mtcnn):
    ...
    if label == "fake":
        ...
        if manipulation_methods:
            ...
            for method_dir in manipulation_methods:
                ...
                print(f"   {method_dir.name}: {len(method_videos)} videos")
        else:
    ...
    print(f"\n📊 Total video folders to process: {total_videos:,}")
...
def main():
    ...
    print("\n" + "="*80)
    print("FF++ FACE EXTRACTION - GPU ACCELERATED (OPTIMIZED)")
    print("="*80)
    ...
    # Process fake videos (all 6 methods)
